...
class testCanBattery(cb.canPassBase):
    def __init__(self):
        ...
        #########################################
        ###### Surachad Re-state of charge ######
        ######           14 Oct 2025       ######
        #########################################
        self.init_max_current = 35.0
        self.init_max_voltage = 54.8

        self.battery_info.max_charge_current = self.init_max_current
        self.battery_info.max_charge_voltage = self.init_max_voltage

        self.was_charging = False
        self.init_published = False
        self.init_pub_timeout_t = mu.Timer(5000)
    ...
    def judgeCanframe(self, msg):
        ...
        if can_id == HB.FRAME_VOLT_CUR_SOC:
            ...
            # print(f"[MAXCHARGE] Voltage: {max_voltage} V, Current: {max_current} A")

            #########################################
            ###### Surachad Re-state of charge ######
            ######           14 Oct 2025       ######
            #########################################
            print(f"[Perc] {self.battery_info.percetage}  [MaxChargeCur]  {self.battery_info.max_charge_current} [MaxChargeVolt]  {self.battery_info.max_charge_voltage}")
            print(
                f"[IsCharging] {self.battery_info.is_charging}  [IsNeedCharge]  {self.isNeedCharge()} ")
            if self.battery_info.charge_current > 3:  # and self.was_charging == False : #  # self.isNeedCharge():
            ...
            if self.battery_info.charge_current <= -0.2 and self.was_charging == True:
            ...
            if self.was_charging:
                if (max_current == 0 or max_voltage == 0) and self.battery_info.percetage <= 0.98:
                    max_current = self.init_max_current
                    max_voltage = self.init_max_voltage
                elif self.battery_info.percetage >= 0.99:
                    pass
                self.battery_info.max_charge_current = max_current
                self.battery_info.max_charge_voltage = max_voltage
                self.init_pub_timeout_t.reset()
                #########################################
                ######           End Edited        ######
                #########################################
            self.msg_ok = True
        ...
        ######           14 Oct 2025       ######
        #########################################
        if (self.init_published and self.init_pub_timeout_t.isTimeUp()):
            self.init_pub_timeout_t.reset()
            self.init_published = False
    ...
    def judgePublish(self):
        # print(f"[IsCharging]  {self.battery_info.is_charging} [MaxChargeCur]  {self.battery_info.max_charge_current} [MaxChargeVolt]  {self.battery_info.max_charge_voltage}")
    ...
    def log_battery_status(self):
        try:
            ...
            snapshot = {
                "Time": timestamp,
                "Voltage": round(getattr(self.battery_info, "charge_voltage", 0), 2),
                "Current": round(getattr(self.battery_info, "charge_current", 0), 2),
                "SOC": round(getattr(self.battery_info, "percetage", 0), 2),
                "Temp": getattr(self.battery_info, "temperature", 0),
                # "IsCharging": getattr(self.battery_info, "is_charging", False),
            }

            log_line = (f"[{snapshot['Time']}] "
                        f"V={snapshot['Voltage']}V | "
                        f"I={snapshot['Current']}A | "
                        f"SOC={snapshot['SOC']}% | "
                        f"T={snapshot['Temp']}°C | ")
            print(log_line)
        except Exception as e:
    ...
    def loop(self):
        ...
        while True:
            ...
            ######           14 Oct 2025       ######
            #########################################
            if ((not self.init_published) and self.init_pub_timeout_t.isTimeUp()):
                print("Init publish max charge values")
                self.battery_info.max_charge_current = self.init_max_current
                self.battery_info.max_charge_voltage = self.init_max_voltage
                # self.publish(self.battery_info)
            #########################################
