...
class HIRANO_BATTERY:
    ...
    FRAME_CONTROL = 0x170  # Robot controls the status of BMS
    FRAME_MAXCHARGE = 0x1F4  # Maximum allowable charging.
    # ---------------- Alarm Codes ---------------- #
    ERROR_CODE_DICT = {
        0xff: "No fault",
        0x17: "Low SOC",
    ...
    CHARGE_CONTROL = {
        0: "Allow Charging",
        1: "No Charging allowed"
    }

    # ---------------- Parser Functions ---------------- #

    @staticmethod
    def parse_volChargeMaxLimit(data):
        if len(data) < 2:
            return None
        raw = (data[0] << 8) | data[1]
        return raw * 0.1
    ...
    @staticmethod
    def parse_curChargeMaxLimit(data):
        if len(data) < 4:
            return None
        raw = (data[2] << 8) | data[3]
        return raw * 0.1
    ...
    @staticmethod
    def parse_chargeControl(data):
        if len(data) < 5:
            return None
        raw = (data[4]) | 0x01
        return {
            "ChargeControl": raw,
            "ChargeControlDesc": HIRANO_BATTERY.CHARGE_CONTROL.get(raw, "Unknown")
        }
